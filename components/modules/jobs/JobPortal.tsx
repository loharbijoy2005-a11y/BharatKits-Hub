...
export default function JobPortal() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      console.warn("Error fetching live jobs from database:", e);
  ...
  // Fetch live jobs from API route on mount
  useEffect(() => {
    fetchJobs();
  }, []);
    ...
    <div className="w-full max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Refresh Database Feed Button */}
          <button
            onClick={fetchJobs}
            disabled={loading}
            title="Fetch latest jobs from database"
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 transition-all disabled:opacity-50 shadow-sm"
          >
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500">
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 border border-emerald-200/80 dark:border-emerald-800/60">
