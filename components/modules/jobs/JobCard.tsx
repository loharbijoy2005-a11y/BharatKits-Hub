...
export function JobCard({
  ...
  const getDeadlineStatus = () => {
    ...
    if (diffDays === 0) {
      return { text: "⚡ Last Day Today!", color: "bg-red-600 text-white animate-pulse border border-red-700" };
    } else if (diffDays <= 4) {
      return {
    ...
    <div
      className={`group relative flex flex-col justify-between rounded-2xl p-5 sm:p-6 transition-all duration-300 bg-white dark:bg-slate-900 border ${
        isClosed
          ...
          : isTeaching
          ? "border-rose-200 dark:border-rose-900/40 hover:border-rose-400 dark:hover:border-rose-600 hover:shadow-xl hover:shadow-rose-500/10"
          : isGovt
          ? "border-amber-200 dark:border-amber-900/40 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-xl hover:shadow-amber-500/10"
          : "border-indigo-200 dark:border-indigo-900/40 hover:border-indigo-400 dark:hover:border-indigo-600 hover:shadow-xl hover:shadow-indigo-500/10"
      }`}
    >
      {/* Top Header Badges */}
      <div>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2 flex-wrap">
            {isTeaching ? (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-black bg-rose-100 dark:bg-rose-950/70 text-rose-900 dark:text-rose-300 border border-rose-300 dark:border-rose-800/80">
                <BookOpen className="w-3.5 h-3.5 text-rose-600 dark:text-rose-400" />
                Teaching / TET
              </span>
            ) : isGovt ? (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-black bg-amber-100 dark:bg-amber-950/70 text-amber-900 dark:text-amber-300 border border-amber-300 dark:border-amber-800/80">
                <Landmark className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" />
                Sarkari / State Govt
              </span>
            ) : (
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-black bg-indigo-100 dark:bg-indigo-950/70 text-indigo-900 dark:text-indigo-300 border border-indigo-300 dark:border-indigo-800/80">
            ...
            {/* Sector / Category Tag */}
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-extrabold bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
              {job.sector || (isGovt ? govtJob?.gov_sector : "Corporate")}
            </span>

            {/* State Tag */}
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-extrabold bg-rose-50 dark:bg-rose-950/60 text-rose-900 dark:text-rose-300 border border-rose-200 dark:border-rose-900/60">
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-extrabold bg-emerald-50 dark:bg-emerald-950/60 text-emerald-900 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800/60">
        ...
        <div className="mb-4">
          <h3
            ...
            className="text-base sm:text-lg font-black text-slate-900 dark:text-white hover:text-amber-600 dark:hover:text-amber-400 transition-colors cursor-pointer line-clamp-2 leading-snug"
          >
            {job.title}
          </h3>

          <div className="mt-1.5 flex items-center gap-2 text-xs sm:text-sm font-extrabold text-slate-800 dark:text-slate-200">
            {isGovt ? (
              <span>🏛️ {govtJob?.department_or_board}</span>
            ) : (
              <span className="flex items-center gap-1.5">
                {privJob?.company_logo_url && (
                  <img
        ...
        <div className="grid grid-cols-2 gap-2.5 mb-4">
          {isGovt && govtJob && (
            <>
              <div className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-xs">
                <GraduationCap className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                <div className="truncate">
                  <div className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Eligibility</div>
              ...
              <div className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-xs">
                ...
                <div className="truncate">
                  <div className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Vacancies</div>
                  <div className="font-extrabold text-slate-900 dark:text-white truncate">
          ...
          {!isGovt && privJob && (
            <>
              <div className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-xs">
                ...
                <div className="truncate">
                  <div className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Salary / CTC</div>
                  <div className="font-extrabold text-slate-900 dark:text-white truncate">
              ...
              <div className="flex items-center gap-2 p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-xs">
                ...
                <div className="truncate">
                  <div className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400">Experience</div>
                  <div className="font-extrabold text-slate-900 dark:text-white truncate">
        ...
        {!isGovt && privJob && privJob.skills_tags && privJob.skills_tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {privJob.skills_tags.slice(0, 4).map((tag, idx) => (
              <span
                key={idx}
                className="px-2 py-0.5 rounded-md text-[11px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="pt-2 flex items-center gap-2.5">
        ...
        {isGovt && govtJob && (
          <a
            ...
            className="flex-1 inline-flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl text-xs font-bold border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 transition-all shadow-sm"
          >
            {govtJob.has_direct_pdf && govtJob.notification_pdf_url ? (
              <>
