...
export function Header({ searchVal, onSearch, onGoHome, showSearch }: HeaderProps) {
  ...
  useEffect(() => {
    // Force clean light mode to prevent washed-out text
    localStorage.setItem("omnikits_theme", "light");
  ...
  return (
    <header className="sticky top-0 z-50 transition-all duration-300">
      ...
      <div className="bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          >
            ...
            <div className="flex flex-col text-left">
              <span className="font-black text-xl tracking-tight text-slate-900">
                Bharat<span className="text-orange-600">Kits</span>
              </span>
          ...
          <div className={`relative max-w-md w-full transition-opacity duration-200 ${showSearch ? "opacity-100" : "opacity-30 pointer-events-none"}`}>
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2 rounded-xl border border-slate-300 bg-slate-50 text-slate-900 text-sm font-bold focus:bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/30 focus:border-orange-500 transition-all placeholder:text-slate-400 shadow-2xs"
            />
            {searchVal && (
              <button
                onClick={() => onSearch("")}
          ...
          <div className="flex items-center gap-3">
            <span className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 border border-emerald-300/80 text-[11px] font-extrabold text-emerald-800 select-none shadow-2xs">
            ...
            <button
              ...
              className="w-10 h-10 rounded-xl hover:bg-orange-50 flex items-center justify-center text-lg border border-slate-200 transition-all text-slate-700 shadow-2xs"
            >
